...
struct PreProcessedBlock
{
    ...
    struct InputPt {
        ...
        TxHash prevoutTxId; ///< 32-byte prevoutHash.  In *reversed* memory order (hex-encoding ready!) (May be a shallow copy of a byte array in `txInfos` if the prevout tx was in this block.). May be empty if coinbase
    ...
    */
    ...
    const TxHash &txIdForOutputIdx(unsigned outputIdx) const {
    ...
    const TxHash &txIdForInputIdx(unsigned inputIdx) const {
        if (inputIdx < inputs.size()) {
